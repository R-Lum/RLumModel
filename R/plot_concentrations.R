...
#'
#' @param sequence.step \code{\link{numeric}} (\bold{required}): step of the simulated sequence which
#' is to plot.
#'
...
#' plot_concentrations(object = model.output, sequence.step = 1)
#'
...
#' plot_concentrations(object = model.output, sequence.step = 1, plot.single = TRUE)
#'
#' @export
plot_concentrations <- function(object, sequence.step, ...){

  if(class(object) != "RLum.Analysis"){
    stop("[plot_concentrations()] Input object is not of type 'RLum.Analysis'")
  }

  if(class(sequence.step) != "numeric"){
    stop("[plot_concentrations()] Argument \"sequence.step\" is not of type 'numeric'")
  }

  if(sequence.step <= 0 | sequence.step > length(object)){
    stop(paste("[plot_concentrations()] Argument \"sequence.step\" has to be non-negative and maximum",length(object)))
  }

  ##get concentrations from object

  concentrations <- lapply(1:length(object),function(x){

    level_concentrations <- object@records[[x]]@info$concentrations
    return(level_concentrations)
  })

  names(concentrations) <- names(object)

  ##check if "TL" is part of the sequence-step to plot, add xlab, ylab
  if(names(object)[sequence.step] == "TL") {
  ...
  ##plot
  Luminescence::plot_RLum.Analysis(concentrations[[sequence.step]],
            xlab = xlab,
            ylab = ylab,
            ...)
