...
#'
#' @param sequence \code{\link{list}} : a list generated by RLumModel_seq2R or handmade
#'
...
#'
#' @noRd
.RLumModel_SequenceTranslator <- function(
  sequence,
  n,
  parms,
  txtProgressBar = TRUE
...
for (i in 1:length(sequence)){
  ...
  if(((n@data$temp < sequence[[i]][1])&&(names(sequence)[i] != "PH")&&(names(sequence)[i] != "CH")) == TRUE){
    n <- .RLumModel_heating(temp_begin = n$temp,temp_end = sequence[[i]][1],b = 5,n,parms)
  }
  ...
  if((names(sequence)[i] == "PH")){
    if(length(sequence[[i]]) == 2){
      n <- .RLumModel_heating(temp_begin = n$temp,temp_end = sequence[[i]][1], b = 5,n,parms)
      n <- .RLumModel_pause(temp = sequence[[i]][1], duration = sequence[[i]][2], n, parms)
    }

    if (length(sequence[[i]]) >= 3){
      n <- .RLumModel_heating(temp_begin = n$temp,temp_end = sequence[[i]][1], b = sequence[[i]][3],n,parms)
      n <- .RLumModel_pause(temp = sequence[[i]][1], duration = sequence[[i]][2], n, parms)
    }
  }
  ...
  if(names(sequence)[i] == "OSL"){
    n <- .RLumModel_CW_OSL(temp = sequence[[i]][1],duration = sequence[[i]][2], optical_power = sequence[[i]][3],n,parms)
    output.model[[i]] <- n$CW_OSL.data
  }

  #check if current sequence step is ILL (illumination)
  if(names(sequence)[i] == "ILL"){
    n <- .RLumModel_illumination(temp = sequence[[i]][1],duration = sequence[[i]][2], optical_power = sequence[[i]][3],n,parms)

  }

  #check if current sequence step is LM_OSL
  if(names(sequence)[i] == "LM_OSL"){
    n <- .RLumModel_LM_OSL(temp = sequence[[i]][1],duration = sequence[[i]][2],n = n,parms = parms)
    output.model[[i]] <- n$LM_OSL.data
  }

  #check if current sequence step is TL
  if(names(sequence)[i] == "TL"){
    n <- .RLumModel_TL(temp_begin = sequence[[i]][1],temp_end = sequence[[i]][2],b = sequence[[i]][3],n,parms)
