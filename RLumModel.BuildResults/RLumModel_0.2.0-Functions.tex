...
1 & ExampleData.FittingTL & Example data with TL curves extracted from a TL-SAR protocol (lab code BT1195) & $<$br /$>$ Example data with TL curves extracted from a TL-SAR protocol (lab code BT1195)$<$br /$>$ & 0.1.0
 ...
 &  &  & $<$br /$>$ Johannes Friedrich, University of Bayreuth (Germany)$<$br /$>$ \\ 
  3 & extract\_pars2FME & Prepare parameters for use with R package FME and function  fit\_RLumModel2data & $<$br /$>$ Prepare parameters for use with R package FME and function  fit\_RLumModel2data $<$br /$>$ & 0.1.1 & [2016-05-24]
 &  & $<$br /$>$ Johannes Friedrich, University of Bayreuth (Germany),$<$br /$>$ \\ 
 ...
 &  & $<$br /$>$ Johannes Friedrich, University of Bayreuth (Germany)$<$br /$>$ \\ 
  5 & model\_LuminescenceSignals & Model Luminescence Signals & $<$br /$>$ This function models luminescence signals for quartz based on published physical models.$<$br /$>$ It is possible to simulate TL, (CW-) OSL, RF measurements in a arbitrary sequence. This$<$br /$>$ sequence is definded as a  list  of certain abrivations. Furthermore it is possible to$<$br /$>$ load a sequence direct from the Riso Sequence Editor.$<$br /$>$ The output is an  RLum.Analysis object and so the plots are done$<$br /$>$ by the  plot\_RLum.Analysis  function. If a SAR sequence is simulated the plot output can be disabled and SAR analyse functions$<$br /$>$ can be used.$<$br /$>$ & 0.1.2 & [2016-05-24]
