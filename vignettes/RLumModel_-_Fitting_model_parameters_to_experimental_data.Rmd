...
The sensitivity analysis as well as the fitting procedure is done using the **R**-package 'FME' by Karline Soetaert and Thomas Petzoldt (https://CRAN.R-project.org/package=FME). For further information the reader is referred to the FME manual and the FME vignettes therein.
...
A new feature from RLumModel version $\geq$ 0.2.3 is the function `fit_RLumModel2data()`. The result from this function is the main function for all sensitivity analysis (local and global). The main arguments of the `fit_RLumModel2data()` function are a sequence to be simulated, a quartz luminescence model and the sequence step in 'sequence' to be analysed. Note that only sequence steps producing a signal output, are allowed. These are: TL, OSL, LM-OSL and RF.
...
As quartz luminescence model the parameter from @Pagonis_2007a are chosen. 
...
* parms: unlisted parameters (done via `extract_parameters2FME()`)
...
var0 <- SF$modVariance
cov0 <- SF$cov.scaled*2.4^2/9

MCMC <- modMCMC(
  f = Fit_func, 
  p = log(parms[c("N1", "N2", "N3", "E1", "E2", "E3", "s1", "s2", "s3")]),
  jump = cov0,
  var0 = var0,
  wvar0 = 1)
...
```{r copy new parameters TL after MCMC, eval=FALSE, include=FALSE}
...
```{r Plot results TL after MCMC, eval=FALSE, fig.align="center", include=FALSE}
...
```{r, echo=FALSE, fig.align = "center"}
plot(SensR, legpos = "topright", xlab = "Time [s]", main = "Local Sensitivity Analysis")
```
...
var0 <- SF$modVariance
cov0 <- SF$cov.scaled *2.4^2/3

MCMC <- modMCMC(
  f = Fit_func, 
  p = log(c(parms[c("N3", "E_th3", "Th3")])),
  jump = cov0, 
  var0 = var0,
  wvar0 = 1)
...
```{r copy new parameters OSL after MCMC, eval=FALSE, include=FALSE}
...
```{r Plot results OSL after MCMC, eval=FALSE, fig.align="center", include=FALSE}
plot(func_FME(parms_after_MCMC),
     type = "l",
     main = "TL Fitting",
     xlab = "Temperature [\u00B0C]",
