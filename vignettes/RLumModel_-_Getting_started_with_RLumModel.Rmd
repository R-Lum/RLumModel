...
All calculations were done with RLumodel 0.2.0 and Luminescence 0.6.0.
...
The first argument required for the function \texttt{model\_LuminescenceSignals()} is the name of a 
quartz luminescence model to be used, respectively the used parameter set in this quartz luminescence model. 
All currently implemented quartz luminescence models were described in Section~\ref{sec:description_models}. 
The command to select a set of parameters from a specific model in \mbox{'RLumModel'} is a character string
...
The second argument in the \texttt{model\_LuminescenceSignals()} function is the sequence to be simulated. 
...
Therefore, the function \texttt{model\_LuminescenceSignals()} offers a special argument 
called \texttt{lab.dose\_rate}, representing the dose rate of the irradiation unit in the laboratory. 
...
with the \texttt{model\_LuminescenceSignals()} function, a loop causing a stepwise increase 
...
The function \texttt{model\_LuminescenceSignals()} is also capable of simulating OSL phenomena.
...
For simulating a DRT, it is necessary to define a sequence with the keyword \texttt{Irr\_2recover}, 
...
\texttt{Luminescence::analyse\_SAR.CWOSL()} and \texttt{Luminescence::plot\_DRTResults()} 
for routine analyses and plotting.

```{r DRT, fig.align = "center"}
##set PH temperatures
...
In the code above, \texttt{plot = FALSE} was chosen, because a single OSL plot is not necessary to 
analyse a SAR sequence. To calculate a $D_e$ from the 
produced \texttt{RLum.Analysis} object \texttt{model.output}, the 
function \texttt{Luminescence::analyse\_SAR.CWOSL()} is suitable.
...
in an \texttt{RLum.Results} object, here termed \texttt{results}. 
