...
```{r solve Chen 2013, fig.align = "center", fig.cap = "TL with parameter sets of Chen 2013"}
...
```{r, echo=FALSE, fig.align = "center", fig.cap = "Concentrations of different energy levels"}
...
The main function to use this feature is `fit_RLumModel2data()`. As in `model_LuminescenceSignals()`, sequence, model and own parameters had to be the parameters. Additional to that, the (required) parameter `seq.step2fit()` is used to choose the sequence step in the defined sequence, which will be analysed. In the example from @Chen_2013d only the sequence step 1 is available and a analysis from this step is expected. Another (optional) parameter is 'norm': If true all results will be normalized to the highest value. This is recommended in fitting experimental data to model parameters, but not when a sensitivity analysis is performed.

To execute a sensitivity analysis with your own parameters, the function `extract_pars2FME()` is necessary to prepare the parameters for the usage with 'RLumModel'.
...
```{r, fig.align = "center", fig.cap = "Local sensitivity analysis with own parameter sets"}
