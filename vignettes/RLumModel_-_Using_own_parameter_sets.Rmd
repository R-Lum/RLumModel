...
With RLumModel $\geq$ 0.2.0 is it possible to simulate quartz luminescence behaviour of own parameter sets or parameter sets, which are not included in the package but also known in literature. Wide spreaded OTOR (One-Trap-One-Recombination-center) models can be included, too. 
...
* Solving these equations is performed with C++ [@Eddelbuettel_2013] and offers a faster calculation.
* Sensitivity analysis and inverse modelling is also possible with own parameter sets (see chapter 2.3.3)
...
When all parameters are set the simulation can be started. The main function in RLumModel is 'model_LuminescenceSignals()' and the usage with own parameter sets is described below. For a general overview for creating a sequence, running RLumModel with stored models etc. the user is referred to @Friedrich_2016 and to the vignette 'RLumModel - Getting started with RLumModel'. For simulating the results of @Pagonis_2009a the follwing sequence is needed.
...
@Chen_2013d published a numerical model to investigate the quasi-equilibrium assumptions in TL. For the descripton of the system a OTOR model was used. 
...
```{r solve Chen 2013, fig.align = "center"}
...
### Sensitivity analysis with own parameters

Sensitivity analysis and inverse modelling with model parameters is one of the big advantages of 'RLumModel'. A manual how to perform a sensitivtiy analysis with 'RLumModel' can be found in the vignette 'RLumModel - Fitting model parameters to experimental data'.
The main function to use is 'fit_RLumModel2data()'. As 'model_LuminescenceSignals()' sequence, model and own parameters had to be the parameters. Additional to that, the (required) parameter 'seq.step2fit' is used to choose the sequence step in the defined sequence, which will be analysed. In the example from @Chen_2013d only the sequence step 1 is available and a analysis from this step is expected. Another (optional) parameter is 'norm': If true all results qill be normalized to the highest value. This is recommended in fitting experimental data to model parameters, but not when a sensitivity analysis is performed.

The function 'extract_pars2FME()' has two possible arguments: \texttt{parms} and \texttt{model}. If a own parameter set is used, the parameter to chose is \texttt{parms}. This is necessary to bring the parameters in the correct form for using the R-package FME.
...
Then, as described in the vignette 'RLumModel - Fitting model parameters to experimental data', different analysis and fitting methods can be performend on the parameter set. 

```{r, eval=FALSE, include=FALSE}
parRanges <- data.frame(min = c(1e-10),
                        max = c(1e-7))

rownames(parRanges) <- c("B2")
...
Quartz as well as $Al_2O_3$ luminescence phenomena can be numerically described and graphically plotted. Another feature is the possibility to perform sensitivity analysis and inverse modelling with own data sets. 
